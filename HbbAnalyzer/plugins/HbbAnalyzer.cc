...
//#ifdef THIS_IS_AN_EVENT_EXAMPLE
  ...
  for(unsigned int iR=0; iR<HbbHandle->genTeleHiggs.size(); iR++) {
    ...
    histograms["Tmjj_gen"]->Fill((HbbHandle->genTeleHiggs[iR].daughters[lSublIndex.first].lv+HbbHandle->genTeleHiggs[iR].daughters[lSublIndex.second].lv).M());
  ...
  for(unsigned int iR=0; iR<HbbHandle->TeleHiggs.size(); iR++) {
    ...
    histograms["Tmjj"]->Fill((HbbHandle->TeleHiggs[iR].daughters[lSublIndex.first].lv+HbbHandle->TeleHiggs[iR].daughters[lSublIndex.second].lv).M());
...
HbbAnalyzer::beginJob()
{
  ...
  //Non-Telescoping Higgs
  histograms["HiggsM_gen"]     = fs->make<TH1D>("HiggsM_gen"     ,"M_{H}^{GEN};M_{H}^{GEN} [GeV];Events / 1 GeV"             ,200,0,200);
  histograms["HiggsPt_gen"]    = fs->make<TH1D>("HiggsPt_gen"    ,"p_{T}_{H}^{GEN};p_{T}_{H}^{GEN} [GeV];Events / 5 GeV"     ,200,0,1000);
  histograms["HiggsEta_gen"]   = fs->make<TH1D>("HiggsEta_gen"   ,"#eta_{H}^{GEN};#eta_{H}^{GEN};Events / 0.1"               ,200,0,200);
  histograms["HiggsPhi_gen"]   = fs->make<TH1D>("HiggsPhi_gen"   ,"#phi_{H}^{GEN};#phi_{H}^{GEN};Events / 6^{#circ}"         ,200,0,200);
  histograms["HiggspdgId_gen"] = fs->make<TH1D>("HiggspdgId_gen" ,"pdgId_{H}^{GEN};pdgId_{H}^{GEN};Number of Jets"           ,200,0,200);
  histograms["mjj_gen"]        = fs->make<TH1D>("mjj_gen"        ,"m_{jj}^{GEN};m_{jj}^{GEN} [GeV];Events / 1 GeV"           ,200,0,200);

  //Non-Telescoping Jets
  histograms["j1M_gen"]        = fs->make<TH1D>("j1M_gen"        ,"M^{GEN Leading};M^{GEN Leading} [GeV];Events / 1 GeV"                ,200,0,200);
  ...
  histograms["j2M_gen"]        = fs->make<TH1D>("j2M_gen"        ,"M^{GEN Sub-Leading};M^{GEN Sub-Leading} [GeV];Events / 1 GeV"        ,200,0,200);
  ...
  //Telescoping Higgs
  histograms["THiggsM_gen"]    = fs->make<TH1D>("THiggsM_gen"     ,"M_{H}^{GEN} (R={0.1-1.5});M_{H}^{GEN} [GeV];Events / 1 GeV"        ,200,0,200);
  ...
  for(unsigned int iR=0; iR<=15; iR++) {
    histograms[Form("THiggsM_gen_R%i",iR)] = fs->make<TH1D>(Form("THiggsM_gen_R%i",iR),Form("M_{H}^{GEN} (Anti-k_{T} R=%.1f, PF+CHS);M_{H}^{GEN} [GeV];Events / 1 GeV",(iR/10.0)+0.1),200,0,200);
  }
  histograms["Tmjj_gen"]       = fs->make<TH1D>("Tmjj_gen"        ,"m_{jj}^{GEN};M_{jj}^{GEN} [GeV];Events / 1 GeV"                          ,200,0,200);

  //Telescoping Jets
  histograms["Tj1M_gen"]       = fs->make<TH1D>("Tj1M_gen"        ,"M^{GEN Leading Jet};M^{GEN Leading Jet} [GeV];Events / 1 GeV"            ,200,0,200);
  ...
  histograms["Tj2M_gen"]       = fs->make<TH1D>("Tj2M_gen"        ,"M^{GEN Sub-Leading Jet};M^{GEN Sub-Leading Jet} [GeV];Events / 1 GeV"    ,200,0,200);
  ...
  //Telescoping Higgs
  histograms["THiggsM"]   = fs->make<TH1D>("THiggsM"  ,"M_{H} (R={0.1-1.5});M_{H} [GeV];Events / 1 GeV"                      ,200,0,200);
  ...
  for(unsigned int iR=0; iR<=15; iR++) {
    histograms[Form("THiggsM_R%i",iR)] = fs->make<TH1D>(Form("THiggsM_R%i",iR),Form("M_{H} (Anti-k_{T} R=%.1f, PF+CHS);M_{H} [GeV];Events / 5 GeV",(iR/10.0)+0.1),200,0,1000);
  }
  histograms["Tmjj"]      = fs->make<TH1D>("Tmjj"     ,"m_{jj};M_{jj} [GeV];Events / 1 GeV"                                  ,200,0,200);

  //Telescoping Jets
  histograms["Tj1M"]      = fs->make<TH1D>("Tj1M"     ,"M^{Leading Jet};M^{Leading Jet} [GeV];Events / 1 GeV"                ,200,0,200);
  ...
  histograms["Tj2M"]      = fs->make<TH1D>("Tj2M"     ,"M^{Sub-Leading Jet};M^{Sub-Leading Jet} [GeV];Events / 1 GeV"        ,200,0,200);
...
HbbAnalyzer::ZSelection() {
  bool massWindowCut = (HbbHandle->theV.lv.M()>70 && HbbHandle->theV.lv.M()<110);
  bool ptCut = HbbHandle->theV.lv.Pt()>50;
